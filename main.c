...
RingBuff *init() {
  RingBuff *new_rb =
      malloc(sizeof(RingBuff)); // ALLOCATE DYNAMIC MEMORY FOR NEW RING BUFFER
                                // (DONT FORGET TO FREE THIS!!!)
  new_rb->read_ptr =
      &(new_rb->arr[0]); // SET READ POINTER TO ADDRESS OF 1ST INDEX OF ARRAY
  new_rb->write_ptr =
      &(new_rb->arr[0]); // SET WRITE POINTER TO ADDRESS OF 1ST INDEX OF ARRAY
  memset(new_rb->arr, 0, BUFFER_SIZE); // INITIALISE ARRAY WITH ALL 0's (MAYBE A
...
void reset(RingBuff *rb) {
  ...
  rb->full = false;
  memset(rb->arr, 0,
         BUFFER_SIZE); // DO WE NEED TO DO THIS SINCE IT WILL OVERWRITE?
}

void freeRB(RingBuff *rb) { free(rb); }

int spaceLeft(RingBuff *rb) {
  int space = 0;
  // CHECK ONLY IF BUFFER IS NOT FULL, SINCE IF IT IS FULL SPACE WILL BE 0
  // (DUH!)
  if (rb->full == false) {
    // IF WRITE POINTER IS AHEAD OF READ POINTER = BUFFER_SIZE - (WRITE_POINTER
    // - READ_POINTER)
    if (rb->write_ptr >= rb->read_ptr) {
      space =
          BUFFER_SIZE - (rb->write_ptr - rb->read_ptr); // Ex: 5 - (4 - 1) = 2

      // IF READ POINTER IS AHEAD OF WRITE POINTER = (READ_POINTER -
      // WRITE_POINTER)
    } else {
...
void resetARB(AtomicRingBuff *rb) {
  ...
  /*
      ...
      No Reordering: Do not move any memory operations that come before this
     line to after this line, and vice versa. Global Visibility: Process change
     to main memory immediately so every other thread sees it instantly. Total
     Order: All threads in the program will agree on the exact order in which
     this operation happened relative to other atomic operations.
  */
  atomic_store_explicit(&rb->head, 0, memory_order_seq_cst);
  atomic_store_explicit(&rb->tail, 0, memory_order_seq_cst);
}

void freeARB(AtomicRingBuff *rb) { free(rb); }
...
void writeARB(AtomicRingBuff *rb, char newChar) {
  ...
  size_t next_head =
      (head + 1) %
      ATOMIC_BUFFER_SIZE; // Smarter way to get index to wrap back to start! Ex:
                          // 7+1 % 8 = 0 (BACK TO START!)
  ...
  rb->arr[head] = newChar;
  // memory_order_release = tells other threads that you can use this slot in
  // arr
...
char readARB(AtomicRingBuff *rb) {
  ...
  // memory_order_release = I am done reading this slot, other threads can
  // overwrite this now!
